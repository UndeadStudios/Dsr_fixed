            ...
            if (players[i].disconnected) {
                for (int i2 = 0; i2 < server.npcHandler.maxNPCs; i2++) {
                    if (server.npcHandler.npcs[i2] != null && players[i] != null) {
                        if (server.npcHandler.npcs[i2].followPlayer
                                == players[i].playerId
                ...
                if (players[i].savefile == true) {
                    if (saveGame(players[i])) {
                        ...
                        savechar(players[i]);
                        System.out.println(
                                "Game saved for player " + players[i].playerName);
                    } else {
                        System.out.println(
                    ...
                    System.out.println(
                            "Did not save for " + players[i].playerName);
                }
            ...
            if (players[i].disconnected) {
                for (int i3 = 0; i3 < server.npcHandler.maxNPCs; i3++) {
                    if (server.npcHandler.npcs[i3] != null && players[i] != null) {
                        if (server.npcHandler.npcs[i3].followPlayer
                                == players[i].playerId) {
    ...
    public void updatePlayer(Player plr, stream str) {
        ...
        for (int i = 0; i < maxPlayers; i++) {
            if (players[i] == null || players[i].isActive == false
                    || players[i] == plr) {// not existing, not active or you are that player
            } else {
                int id = players[i].playerId;

                if (plr.didTeleport == false
                        && (plr.playerInListBitmap[id >> 3] & (1 << (id & 7)))
                                != 0) {// player already in playerList
                } else if (plr.withinDistance(players[i]) == false) {// out of sight
                } else {
            ...
            for (int i = 0; i < plr.friends.length; i++) {
                if (plr.friends[i] > 0) {
                    ...
                    characterfile.write(Integer.toString(i), 0,
                            Integer.toString(i).length());
                    characterfile.write("	", 0, 1);
                    characterfile.write(Long.toString(plr.friends[i]), 0,
                            Long.toString(plr.friends[i]).length());
                    characterfile.newLine();
                }
            }
            characterfile.newLine();
            ...
            for (int i = 0; i < plr.ignores.length; i++) {
                if (plr.ignores[i] > 0) {
                    ...
                    characterfile.write(Integer.toString(i), 0,
                            Integer.toString(i).length());
                    characterfile.write("	", 0, 1);
                    characterfile.write(Long.toString(plr.ignores[i]), 0,
                            Long.toString(plr.ignores[i]).length());
                    characterfile.newLine();
                }
            }
            characterfile.newLine();
        ...
        try {
            ObjectOutputStream out = new ObjectOutputStream(
                    new FileOutputStream(
                            "./savedGames/" + tempSave.playerName + ".dat"));
